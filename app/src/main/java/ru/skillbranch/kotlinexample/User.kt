...
class User private constructor(
    ...
    constructor(
        ...
        firstName, lastName, email = email, meta = mapOf("auth" to "password")
    ) {
    ...
    private fun encrypt(password: String): String {
        ...
        println("Salt while encrypte: $salt")
    ...
    fun generateAccessCode(): String {
        val possible = "" //TODO нужно посвотреть на SkillBranch возможно есть этот текст
    ...
    fun sendAccessCodeToUser(phone: String, code: String) {
        println("..... sending access codeL $code on $ $phone")
    }

    companion object Factody {
        fun makeUser(
            fullName: String,
            email: String? = null,
            ...
            this.split(" ")
                ...
                .run {
                    when (size) {
                        1 -> first() to null
                        2 -> first() to last()
                        else -> throw IllegalArgumentException("FullName mut contain only first name and last name, current split result: ${this@fullNameToPair}")
                    }
                }
